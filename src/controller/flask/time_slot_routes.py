    ...
    def add_time_range():
        try:
            ...
            if day not in _VALID_DAYS:
                return jsonify(
                    {
                        "error": f"'{day}' is not a valid day. Use MON, TUE, WED, THU, or FRI."
                    }
                ), 400
            ...
            if spacing is None or int(spacing) <= 0:
                return jsonify(
                    {"error": "Spacing must be a positive integer (minutes)."}
                ), 400

            scheduler.time_slot.add_time(
                scheduler.config, day, start, int(spacing), end
            )
    ...
    def modify_time_range(day, index):
        try:
            ...
            if not scheduler.time_slot.validate_time_entry(
                ...
                return jsonify(
                    {"error": f"Time range index {index} for {day} not found."}
                ), 404

            scheduler.time_slot.modify_time(
                scheduler.config, day, index, start, int(spacing), end
            )
    ...
    def add_class_pattern():
        try:
            ...
            if not scheduler.time_slot.validate_class_entry(
                ...
                return jsonify(
                    {"error": "Invalid class pattern — check day names and durations."}
                ), 400
    ...
    def modify_class_pattern(index):
        try:
            ...
            if not scheduler.time_slot.validate_class_entry(
                ...
                return jsonify(
                    {"error": f"Class pattern index {index} not found."}
                ), 404
