...
def validate_time_slot(slot):
    ...
    try:
        ...
        if end_total_mins <= start_total_mins:
            return (
                False,
                f"End time ({end_hour:02d}:{end_min:02d}) must be after start time ({start_hour:02d}:{start_min:02d})",
            )
...
def del_faculty(sched):
    try:
        ...
        sched.faculty.delete_faculty(sched.config, name)
    ...
    try:
        ...
        while True:
            ...
            if not maximum_credits or not maximum_credits.isnumeric():
                print(
                    f"Error: Maximum credits must be a positive integer, got '{maximum_credits}'. Please try again."
                )
                continue
            maximum_credits = int(maximum_credits)
            if maximum_credits <= 0:
                print(
                    "Error: Maximum credits must be greater than 0. Please try again."
                )
                continue
            break
        ...
        while True:
            ...
            if not minimum_credits or not minimum_credits.isnumeric():
                print(
                    f"Error: Minimum credits must be a non-negative integer, got '{minimum_credits}'. Please try again."
                )
                continue
            ...
            if minimum_credits > maximum_credits:
                print(
                    f"Error: Minimum credits ({minimum_credits}) cannot exceed maximum credits ({maximum_credits}). Please try again."
                )
                continue
            break
        ...
        while True:
            ...
            if not unique_course_limit or not unique_course_limit.isnumeric():
                print(
                    f"Error: Unique course limit must be a positive integer, got '{unique_course_limit}'. Please try again."
                )
                continue
            unique_course_limit = int(unique_course_limit)
            if unique_course_limit <= 0:
                print(
                    "Error: Unique course limit must be greater than 0. Please try again."
                )
                continue
            break
        ...
        while True:
            ...
            if not maximum_days or not maximum_days.isnumeric():
                print(
                    f"Error: Maximum days must be an integer between 1-5, got '{maximum_days}'. Please try again."
                )
                continue
            maximum_days = int(maximum_days)
            if maximum_days < 1 or maximum_days > 5:
                print(
                    f"Error: Maximum days must be between 1 and 5, got {maximum_days}. Please try again."
                )
                continue
            break
        ...
        for day in ["MON", "TUE", "WED", "THU", "FRI"]:
            ...
            while True:
                ...
                if slot in slots:
                    print(
                        f"Error: Time slot '{slot}' already added for {day}. Please try again."
                    )
                    continue
                ...
                if not no_overlap:
                    print(
                        f"Error: Time slot '{slot}' overlaps with existing slot '{overlapping_slot}'. Please try again."
                    )
                    continue
        ...
        while True:
            ...
            while True:
                ...
                if not weight or not weight.isnumeric():
                    print(
                        f"Error: Weight must be a positive integer, got '{weight}'. Please try again."
                    )
                    continue
                weight = int(weight)
                break
        ...
        while True:
            ...
            if day.upper() not in days:
                print(
                    f"Error: Invalid day '{day}'. Must be MON/TUE/WED/THU/FRI. Please try again."
                )
                continue
