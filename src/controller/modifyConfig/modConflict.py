...
def del_conflict(sched):
    try:
        ...
        if not sched.conflict.validate_entry(
            sched.config, course_id, "delete", conflicting_course_id
        ):
            return

        sched.conflict.delete_conflict(sched.config, course_id, conflicting_course_id)
...
def add_conflict(sched):
    try:
        ...
        if not sched.conflict.validate_entry(
            sched.config, course_id, "add", conflicting_course_id
        ):
            return

        sched.conflict.add_conflict(sched.config, course_id, conflicting_course_id)
...
def mod_conflict(sched):
    try:
        ...
        if not sched.conflict.validate_entry(
            sched.config, course_id, "modify", old_conf_course_id
        ):
            return
        ...
        if not sched.conflict.validate_entry(
            sched.config, course_id, "add", new_conf_course_id
        ):
            return

        sched.conflict.modify_conflict(
            sched.config, course_id, old_conf_course_id, new_conf_course_id
        )
