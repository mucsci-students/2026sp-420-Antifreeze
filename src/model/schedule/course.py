...
class course:
    ...
    def validate_entry(self, config, course_id: str, operation: str) -> bool:
        ...
        if operation == "add":
            if course_id.upper() in courses:
                print(
                    f"Error: Course '{course_id}' already exists — returning to menu."
                )
                return False
            ...
            if course_id.upper() not in courses:
                print(
                    f"Error: Course '{course_id}' does not exist — returning to menu."
                )
                return False
    ...
    def delete_course(self, config, id: str):
        ...
        for course in courses:
            if id in course.conflicts:
                course.conflicts = [
                    c for c in course.conflicts if c.upper() != id.upper()
                ]
    ...
    def modify_course(
        ...
        for course in courses:
            course.conflicts = [
                new_id if c.upper() == old_id.upper() else c for c in course.conflicts
            ]
        ...
        print(
            f"Course '{old_id}' modified to '{new_id}' successfully (cascade applied)."
        )
    ...
    # Get Course IDs
    # Return Dictionary of courses sorted by course
    # Parameters: CSV formatted file
    # Returns: List of all courses sorted by course
